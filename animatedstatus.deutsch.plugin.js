...
class AnimatedStatus {
	...
	getDescription() {
		return "Animiere deinen Discord-Status";
	}
	...
	Status_Animate(index = 0) {
		...
		if (this.animation[index] == undefined) {
			BdApi.showToast("BetterStatus-Deutsch: Kein Status gesetzt. Gehe zu Einstellungen>Plugins um eine Animation hinzuzufügen!");
			return;
		}
	...
	newRawEdit(str = "") {
		...
		out.placeholder = '"Test (Nachricht)"\n"Test (Nachricht)", "👍 (Symbol)"\n"Test (Nachricht)", "emoji (Nitro-Symbol)", "000000000000000000 (Nitro-Symbol-ID)"\n"eval new String(\'test\') (Javascript)"\n"eval new String(\'test\') (Javascript)", "eval new String(\'👍\') (Javascript)"\n...';
		out.value = str;
		return out;
	}
	...
	getSettingsPanel() {
		...
		// timeout
		settings.appendChild(GUI.newLabel("Zeit per Keyframe (In Millisekunden)"));
		...
		timeout.addEventListener("focusout", () => {
			if (parseInt(timeout.value) < this.kMinTimeout) {
				...
				BdApi.showToast(`Timeout muss niedriger sein als ${this.kMinTimeout}`, {type: "error"});
			}
		});
		...
		addStep.title = "Stufe am Ende hinzufügen";
		addStep.onclick = () => {
		...
		delStep.title = "Letzte Stufe entfernen";
		...
		let changeEditMode = GUI.newButton("Bearbeitungsmodus ändern");
		...
		save.onclick = () => {
			...
			// Show Toast
			BdApi.showToast("Einstellungen wurden gespeichert!", {type: "success"});
